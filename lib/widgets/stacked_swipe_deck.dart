...
class _StackedSwipeDeckState extends State<StackedSwipeDeck>
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    if (_dragOffset.dx > 150 || velocityX > 400) {
      _dismissCard(toRight: true);
    } else if (_dragOffset.dx < -150 || velocityX < -400) {
  ...
  Widget build(BuildContext context) {
    ...
    final double progress = (activeOffset.dx.abs() / 150.0).clamp(0.0, 1.0);

    // Dynamic rotation angle of top card
    final double rotationAngle = (activeOffset.dx / screenWidth) * 0.25;

    // Badges opacities
    final double keepOpacity = (activeOffset.dx / 150.0).clamp(0.0, 1.0);
    final double deleteOpacity = (-activeOffset.dx / 150.0).clamp(0.0, 1.0);
    ...
    return GestureDetector(
      ...
      child: Stack(
        ...
        children: [
          ...
          // 1. Active Gesture Card (index 0) - Centered and reactive
          SwipeCard(
